...
def main():
    parser = argparse.ArgumentParser(
        description="Create a graph that scales out the number of dimension while holding the number of points constant."
    )
    ...
    parser.add_argument(
        "--max_dims", "--max-dims", type=int, help="A max number of dimensions to plot, overwrites the --dims if used."
    )
    ...
    ax.set_title(
        "\n".join(
            textwrap.wrap(
                f"Pairwise Manhattan Distance Timing vs Number of Dimensions in a Point calculated for {args.points} Points",
                50,
            )
        )
    )
