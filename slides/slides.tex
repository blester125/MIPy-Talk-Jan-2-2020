...
def pairwise_manhattan_python_v2(
    ...
    \note<1>{
        \begin{itemize}
            \item This is the only algorithmic improvemnt (not really, Big O is still the same but we are cutting out work) to our code
...
def pairwise_manhattan_numpy_v4(points: np.ndarray) -> np.ndarray:
    ...
    \note<2>{
        \begin{itemize}
            \item Now that we are double broadcasting there is no way to express the fact that we can skip the repeated work of calculating the distance between flipped points.
        \end{itemize}
    }
    \note<3->{
        \begin{itemize}
            ...
            \item We have this itermediate result of size \texttt{[N, N, M]} so our memory usage is higher
            \item We'll see that this extra memory usage can cause problems later.
        \end{itemize}
    }
\end{frame}
...
\begin{frame}
    ...
    \note{
        \begin{itemize}
            ...
            \item However when we scale out this solution doesn't work well
            \item TODO Give memory sizes
            \item speeds weren't meaningfully different from the single broadcast
...
\begin{frame}[fragile]
    ...
    \note{
        \begin{itemize}
            ...
            \item buffer interface is way that objects will organize their memory layout so other things can access the content. NumPy arrays support this interface
            \item results is actually a dynamically created numpy array, it it known to the python interpreter, the gil is needed to make it, the ref counter is incremented by one when we make it, and result will be safe to use in python functions after we erturn it. Making it so easy to create python objects is a strength of cython
...
\begin{frame}
    ...
    \begin{itemize}
        \item Cython is a compilied Language
