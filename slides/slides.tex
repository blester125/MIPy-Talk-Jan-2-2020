...
def pairwise_manhattan_numpy_broadcast_cached(
    ...
    \note{
        \begin{itemize}
            ...
            \end{itemize}
            \item We transpose (the rows become columns and columns rows) and add. Because we are upper triangle we just copy the values over (the previous values are zero)
