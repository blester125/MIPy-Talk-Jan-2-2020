    ...
    \note{
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                \item a list of scalars is an array (or vector)
...
\begin{frame}
    ...
    \note{
        \begin{itemize}
            ...
            \item The absolute difference is the same when reversed so the sum is of the same values to the total sum is the same
        \end{itemize}
        \begin{itemize}
...
def pairwise_manhattan_numpy_v4(points: np.ndarray) -> np.ndarray:
    ...
    \note<2>{
        \begin{itemize}
            \item This idea of double broadcasting is very powerful and is the corner stone of why the Conditional Random Field (CRF) (which is a NLP model) I wrote for mead-baseline is so fast.
...
\begin{frame}
    \makebox[\textwidth][c]{
        \includegraphics[width=0.9\paperwidth]{images/double-broadcast.jpg}
    }
    \note{
        \begin{itemize}
            \item This is an example of double broadcasting
            \item The original points are an $N \text{ x } M$
            \item The second item is the points expanded to $N \text{ x } 1 \text{ x } M$
            \item The third item is the points expanded to $1 \text{ x } N \text{ x } M$
            \item The forth item is the broadcast of $N \text{ x } 1 \text{ x } M \rightarrow N \text{ x } N \text{ x } M$. We can see that the $N$ broadcast results in the duplication of each point into a list of $N$ copies.
            \item The fifth item is the broadcast of $1 \text{ x } N \text{ x } M \rightarrow N \text{ x } N \text{ x } M$. We can see the the broadcast of initial $1 \rightarrow N$ creates $N$ copies of the sets of points.
            \item Now that we have matching we can see that the first item the first point vs all other points, the next item is the second point vs the rest and so one.
        \end{itemize}
    }
\end{frame}
