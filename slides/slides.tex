...
def pairwise_manhattan_numpy_v3(points: np.ndarray) -> np.ndarray:
    ...
    \note<4>{
        \begin{itemize}
            ...
            \item It is possible to remove this redundency but it will get a lottle weird
        \end{itemize}
    }
\end{frame}
...
def pairwise_manhattan_numpy_v4(points: np.ndarray) -> np.ndarray:
    ...
    \note<2>{
        \begin{itemize}
            \item Now that we are double broadcasting there is no way to express the fact that we can skip the repeated work of calculating the distance between symetric pairs of points.
        \end{itemize}
    }
    \note<3->{
        \begin{itemize}
...
\begin{frame}
    ...
    \note{
        \begin{itemize}
            ...
            \item However when we scale out this solution doesn't work well due to memory usage
            \begin{center}
            ...
            \item I could only get this version to work for limited points and only a few dimensions.
