...
\begin{frame}<1>[fragile, label=numpy_md]
    ...
    \note<2>{
        \begin{itemize}
            ...
            \item The subtraction and the \tt{np.abs} is a vectorized function
            \item The \tt{np.sum} is a reduction
        \end{itemize}
    }
\end{frame}
...
def pairwise_manhattan_numpy_v3(points: np.ndarray) -> np.ndarray:
    ...
    \note<2>{
        \begin{itemize}
            ...
            \item The result of broadcasting vectorization for points is \tt{[N, M]}
            \item There is this new kwarg for \tt{np.sum}, \tt{axis}
            \item This is a way to apply reductions to multi dimensional inputs
        \end{itemize}
    }
    \note<3>{
        \begin{itemize}
            \item Reducing \tt{[N, M]} over axis 1 gives us \tt{N}
            \item Results is a list of vectors of size $N$
            \item \tt{np.stack} puts them on top of each other to form a single matrix
...
\begin{frame}
    ...
    \note{
        \begin{itemize}
            ...
            \item There is also a \tt{keepdims} argument to keep the sinlge dimension of size 1 so instead of getting a vector of size $3$ you get one of size \tt{[1, 3]}
        \end{itemize}
    }
\end{frame}
...
def pairwise_manhattan_numpy_v4(points: np.ndarray) -> np.ndarray:
    ...
    \note{
        \begin{itemize}
            ...
            \item When we do the subtraction the points \tt{[N, M]} is expanded to \tt{[1, N, M]}
            \item \tt{[N. 1. M]} + \tt{[1, N, N]} $\rightarrow$ \tt{[N, N, M]}
            \item Reduction over axis $-1$ (same as negative indexing in python, sum over the last dim)
            \item Reduction gives us \tt{[N, N]} aka our answer
        \end{itemize}
    }
\end{frame}
...
\begin{frame}[fragile]
    ...
    \note{
        \begin{itemize}
            ...
            \item \tt{wrapsround} turns off the ability to do negative indexing
            \item \tt{boundscheck} turns off rasing an error when you access past the end of an array
...
\begin{frame}[fragile]
    ...
    \note{
        \begin{itemize}
            \item \tt{cpdef} is a utility to make this function callable by python code and other cython code. C function calls are faster and there is slight overhead from the \tt{cpdef}. Cython in a tight inner loop (like a function of manhattan distance) should use \tt{cdef}
            \item \tt{int[:, :]} before the function name is the return type
...
\begin{frame}[fragile]
    ...
    \note{
        \begin{itemize}
            \item \tt{cdef} is used to statically declare variables. This tells the compiler what the type is so it can compile it to C.
            \item \tt{int[:, :]} is a typed memoryview
...
\begin{frame}
    \frametitle{Timing vs the number of points}
\end{frame}
